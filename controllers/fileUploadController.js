...
import { FileMessages } from "../constants/enums.js";
...
export const uploadFileToS3 = multer({
  storage: multerS3({
    ...
    key: function (req, file, cb) {
      cb(null, 'files/' + Date.now().toString() + '_' + file.originalname);
    },
  }),
  limits: { fileSize: 1024 * 1024 * 25 }, // 5MB file size limit
});
  ...
  try {
    ...
    s3.getSignedUrl('getObject', params, (err, url) => {
      if (err) {
        ...
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: FileMessages.CANNOT_GENERATE_URL });
      }
